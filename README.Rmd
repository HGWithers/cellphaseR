...
An example dataset `PIdata` is provided with the package that contains imaging data for PI stained cells from two different cell types: wildtype (WT) mouse embryonic fibroblasts (wtMEF) or p53 knockout MEFs transduced with HRas-V12-GFP oncogene.
...
```{r cpridges_base_example, dpi = 300}
cpridges(data = PIdata, signal = `PI Obj Integral`, group = Cells)
```

The position of peaks can be labeled using the `label.peaks` argument of `cpridges` and refined using the `peak.threshold` argument.
